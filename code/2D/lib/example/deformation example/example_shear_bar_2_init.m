global ver_num tri_num obj_tri exam_mesh dirichlet en_type cmin cmax K q q_rest
...
q_rest(2 * anchors_fix) = q_rest(2 * anchors_fix) - 0.5;

q_rest(2 * anchors_move - 1) = q_rest(2 * anchors_move - 1) + 2;
q_rest(2 * anchors_move) = q_rest(2 * anchors_move) - 1.5;

q(find(dirichlet)) = q_rest(find(dirichlet));

q_rest = reshape(exam_mesh(1).v(:, 1:2)', ver_num * 2, 1);
