...
fix_anchor = find(tet_mesh.v(:, 2) < -50);
move_anchor1 = intersect(find(tet_mesh.v(:, 2) > 43), find(tet_mesh.v(:, 3) < -870));
move_anchor2 = intersect(intersect(find(tet_mesh.v(:, 2) > 33), find(tet_mesh.v(:, 3) > -870)), find(tet_mesh.v(:, 1) < 0));
move_anchor3 = intersect(intersect(find(tet_mesh.v(:, 2) > 33), find(tet_mesh.v(:, 3) > -870)), find(tet_mesh.v(:, 1) > 0));
...
q_move(3 * move_anchor1 - 1) = V(:, 2) - 50;
q_move(3 * move_anchor1 - 0) = V(:, 3) - 60;
...
V = bsxfun(@plus,bsxfun(@minus,V,ctr)*rot,ctr);
q_move(3 * move_anchor2 - 2) = V(:, 1) - 50;
q_move(3 * move_anchor2 - 1) = V(:, 2) - 50;
q_move(3 * move_anchor2 - 0) = V(:, 3);
...
V = bsxfun(@plus,bsxfun(@minus,V,ctr)*rot,ctr);
q_move(3 * move_anchor3 - 2) = V(:, 1) + 50;
q_move(3 * move_anchor3 - 1) = V(:, 2) - 50;
q_move(3 * move_anchor3 - 0) = V(:, 3);
