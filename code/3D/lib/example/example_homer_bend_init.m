...
load_input_mesh('../data/homer_bend');
...
fix_anchor1 = intersect(find(tet_mesh.v(:, 2) < -0.45), find(tet_mesh.v(:, 1) < 0));
fix_anchor2 = intersect(find(tet_mesh.v(:, 2) < -0.45), find(tet_mesh.v(:, 1) > 0));

move_anchor1 = intersect(intersect(find(tet_mesh.v(:, 3) > 0.1), find(tet_mesh.v(:, 2) > 0)), intersect(find(tet_mesh.v(:, 2) < 0.15), find(tet_mesh.v(:, 1) > 0)));
move_anchor2 = intersect(intersect(find(tet_mesh.v(:, 3) > 0.1), find(tet_mesh.v(:, 2) > 0)), intersect(find(tet_mesh.v(:, 2) < 0.15), find(tet_mesh.v(:, 1) < 0)));
move_anchor3 = find(tet_mesh.v(:, 2) > 0.4);
...
q_move(3 * move_anchor1 - 2) = V(:, 1);
q_move(3 * move_anchor1 - 1) = V(:, 2) - 0.4;
q_move(3 * move_anchor1 - 0) = V(:, 3) + 0.4;
...
V = bsxfun(@plus,bsxfun(@minus,V,ctr)*rot,ctr);
q_move(3 * move_anchor2 - 2) = V(:, 1);
q_move(3 * move_anchor2 - 1) = V(:, 2) - 0.4;
q_move(3 * move_anchor2 - 0) = V(:, 3) + 0.4;
...
rot2 = [1 0 0; 0 cos(theta2) -sin(theta2); 0 sin(theta2) cos(theta2)];
rot = rot1 * rot2;

V = bsxfun(@plus,bsxfun(@minus,V,ctr)*rot,ctr);
q_move(3 * move_anchor3 - 2) = V(:, 1);
q_move(3 * move_anchor3 - 1) = V(:, 2) - 0.3;
q_move(3 * move_anchor3 - 0) = V(:, 3) + 0.4;
